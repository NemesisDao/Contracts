...
contract NemesisTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SNMS }
    ...
    address public sNMS;
    uint public sNMSQueue; // Delays change to sNMS address
    ...
    /**
        @notice allow approved address to deposit an asset for NMS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NMS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( NMS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NMS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INMSERC20( NMS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sNMS ).balanceOf( msg.sender ); // Can only borrow against sNMS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NMS
        @param _amount uint
     */
    function repayDebtWithNMS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INMSERC20( NMS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, NMS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( NMS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( NMS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NMS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NMS decimals
            value_ = _amount.mul( 10 ** IERC20( NMS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SNMS ) { // 9
            sNMSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
